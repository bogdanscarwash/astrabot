...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate a trained Astrabot model"
    )
...
def evaluate_style_similarity(model, tokenizer, test_data: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    if generated_responses:
        ...
        style_metrics["avg_response_length"] = 1 - abs(avg_gen_len - avg_exp_len) / max(avg_gen_len, avg_exp_len)
        ...
        style_metrics["burst_detection_accuracy"] = 1 - abs(burst_gen - burst_exp) / max(len(generated_responses), 1)
        ...
        style_metrics["emoji_usage_similarity"] = 1 - abs(emoji_gen - emoji_exp) / max(len(generated_responses), 1)
