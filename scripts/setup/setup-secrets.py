...
# Add parent directories to path (script is now in scripts/setup/)
...
def main():
    ...
    current_openai = (
        ...
        and env_vars["OPENAI_API_KEY"] != "your-openai-api-key-here"  # pragma: allowlist secret
        else "not set"
    )
    ...
    current_anthropic = (
        ...
        and env_vars["ANTHROPIC_API_KEY"]
        != "your-anthropic-api-key-here"  # pragma: allowlist secret
        else "not set"
    )
    ...
    new_id = get_secure_input(
        f"Your Signal recipient ID (current: {current_id}): ", is_password=False
    )
    if new_id:
    ...
    new_debug = get_secure_input(
        f"Enable debug mode? true/false (current: {current_debug}): ", is_password=False
    )
    if new_debug:
    ...
    new_img = get_secure_input(
        f"Enable image processing? true/false (current: {current_img}): ", is_password=False
    )
    if new_img:
