...
class Qwen3Trainer:
    ...
    def load_and_prepare_data(
        self, messages_path: str, recipients_path: str, your_recipient_id: int = 2
    ...
    def create_mixed_dataset(
        self, messages_df: pd.DataFrame, recipients_df: pd.DataFrame, your_recipient_id: int = 2
    ) -> list[dict]:
        ...
        if self.config["dataset"]["modes"]["conversational"]["enabled"]:
            ...
            conv_data = creator.create_conversational_training_data(
                ...
                include_metadata=self.config["dataset"]["modes"]["conversational"][
                    "include_metadata"
                ],
            )
        ...
        if self.config["dataset"]["modes"]["burst_sequence"]["enabled"]:
            ...
            burst_data = creator.create_burst_sequence_data(
                messages_df.to_dict("records"), your_recipient_id
            )
        ...
        for example in examples:
            ...
            if "messages" in example:
                ...
                if "adapt" in example.get("instruction", "").lower():
                    ...
                    # Simple format
                    messages.append(
                        {"role": "user", "content": example.get("input", example["instruction"])}
                    )
            ...
            formatted_examples.append({"text": text, "metadata": example.get("metadata", {})})
    ...
    def train(self, dataset: Dataset, output_dir: str = "./output"):
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                f"Memory for training: {used_memory_for_lora} GB ({lora_percentage}% of max)"
            )
    ...
    def save_model(self, output_dir: str = "./output"):
        ...
        if self.config["output"]["save_merged_16bit"]:
            ...
            self.model.save_pretrained_merged(
                merged_dir, self.tokenizer, save_method="merged_16bit"
            )
        ...
        if self.config["output"]["gguf"]["enabled"]:
            for method in self.config["output"]["gguf"]["quantization_methods"]:
                ...
                self.model.save_pretrained_gguf(
                    gguf_path, self.tokenizer, quantization_method=method
                )
    ...
    def test_model(self, test_prompts: list[str] = None):
        ...
        for prompt in test_prompts:
            ...
            needs_reasoning = any(
                word in prompt.lower() for word in ["explain", "solve", "why", "how"]
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="./output", help="Output directory for trained model"
    )

    parser.add_argument(
        "--your-id", type=int, default=2, help="Your recipient ID in Signal database"
    )

    parser.add_argument("--multi-stage", action="store_true", help="Run multi-stage training")
    ...
    messages_df, recipients_df = trainer.load_and_prepare_data(
        args.messages, args.recipients, args.your_id
    )

    if args.multi_stage:
        # Run multi-stage training
        trainer.run_multi_stage_training(messages_df, recipients_df, args.your_id, args.output)
    else:
        # Create training dataset
        training_examples = trainer.create_mixed_dataset(messages_df, recipients_df, args.your_id)
