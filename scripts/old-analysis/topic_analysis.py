            ...
            academic_discussions.append(
                {
                    ...
                    "text": body,
                    "academic_indicators": [
                        ind for ind in academic_indicators if ind in body_lower
                    ],
...
def identify_conversation_threads(messages_df: pd.DataFrame) -> dict[str, Any]:
    ...
    for thread_id in messages_df["thread_id"].unique():
        ...
        time_span = (
            (thread_messages["date_sent"].max() - thread_messages["date_sent"].min()) / 1000 / 3600
        )  # hours
...
def generate_topic_report(
    ...
    political_coverage = (
        political_analysis["total_political_messages"] / total_messages if total_messages > 0 else 0
    )
    personal_coverage = (
        personal_analysis["total_personal_messages"] / total_messages if total_messages > 0 else 0
    )
    humor_coverage = (
        humor_analysis["total_humor_messages"] / total_messages if total_messages > 0 else 0
    )
    ...
    report = {
        ...
        "conversation_threads": (
            thread_analysis if detailed else {k: v for k, v in list(thread_analysis.items())[:5]}
        ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze topics and themes in Signal conversations"
    )
    parser.add_argument(
        "--min-messages", type=int, default=10, help="Minimum messages per topic category"
    )
    ...
    try:
        ...
        print(
            f"🏛️  Political Discussions: {political['total_political_messages']:,} ({meta['analysis_coverage']['political_discussions']:.1%})"
        )
        print(
            f"🎯 Personal Interests: {personal['total_personal_messages']:,} ({meta['analysis_coverage']['personal_interests']:.1%})"
        )
        print(
            f"😂 Humor Content: {humor['total_humor_messages']:,} ({meta['analysis_coverage']['humor_content']:.1%})"
        )
        print(f"🔗 URLs Shared: {social['total_urls']:,}")

        print(f"\n🏛️  Political Categories:")
        for category, count in sorted(
            political["category_stats"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for category, count in sorted(
            personal["category_stats"].items(), key=lambda x: x[1], reverse=True
        ):
            if count >= args.min_messages:
                print(f"   {category}: {count:,}")
        ...
        print(f"\n😂 Humor Categories:")
        for category, count in sorted(
            humor["category_stats"].items(), key=lambda x: x[1], reverse=True
        ):
            if count >= args.min_messages:
                print(f"   {category}: {count:,}")
        ...
        for platform, count in sorted(
            social["platform_sharing"].items(), key=lambda x: x[1], reverse=True
        ):
            if count > 0:
