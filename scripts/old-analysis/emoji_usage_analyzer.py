...
def analyze_emoji_by_sender(
    ...
    sender_emoji_patterns = defaultdict(
        lambda: {
            "total_emojis": 0,
            "unique_emojis": set(),
    ...
    for sender_id in messages_df["from_recipient_id"].unique():
        ...
        if sender_data["total_emojis"] > 10:  # Only if they use emojis regularly
            ...
            sender_data["signature_emojis"] = [
                emoji_char for emoji_char, count in most_used if count >= 3
            ]
...
def analyze_temporal_emoji_patterns(messages_df: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "monthly_evolution": {
            month: dict(counter) for month, counter in monthly_emoji_usage.items()
        },
...
def generate_emoji_report(
    messages_df: pd.DataFrame, recipients_df: pd.DataFrame, detailed: bool = False
) -> dict[str, Any]:
    ...
    messages_with_emojis = sum(
        1
        for _, row in messages_df.iterrows()
        if isinstance(row.get("body"), str)
        and any(char in emoji.EMOJI_DATA for char in row.get("body", ""))
    )
    ...
    # Most common emojis
    emoji_frequency = [
        (emoji_char, data["total_count"]) for emoji_char, data in context_patterns.items()
    ]
    ...
    report = {
        ...
        "context_patterns": (
            context_patterns if detailed else {k: v for k, v in list(context_patterns.items())[:10]}
        ),
...
def create_emoji_visualizations(
    report: dict[str, Any], output_dir: str = "data/processed/emoji_plots"
):
    ...
    try:
        ...
        if emotion_dist:
            ...
            filtered_data = [
                (emotion, count) for emotion, count in zip(emotions, counts) if count > threshold
            ]
            ...
            if filtered_data:
                ...
                wedges, texts, autotexts = ax.pie(
                    counts, labels=emotions, autopct="%1.1f%%", startangle=90
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze emoji usage patterns in Signal conversations"
    )
    parser.add_argument(
        "--detailed", action="store_true", help="Include detailed context analysis for all emojis"
    )
    parser.add_argument(
        "--visualize", action="store_true", help="Create visualizations (requires matplotlib)"
    )
    ...
    try:
        ...
        print(
            f"😊 Messages with Emojis: {meta['messages_with_emojis']:,} ({meta['emoji_usage_rate']:.1%})"
        )
        ...
        for emotion, count in sorted(
            emot["emotion_distribution"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
