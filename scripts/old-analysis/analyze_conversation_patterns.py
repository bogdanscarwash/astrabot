...
def load_signal_data(data_dir: str = "data/raw/signal-flatfiles") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def analyze_message_timing(messages_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        'avg_burst_length': sum(b['length'] for b in burst_sequences) / len(burst_sequences) if burst_sequences else 0,
        'longest_burst': max(burst_sequences, key=lambda x: x['length']) if burst_sequences else None,
...
def extract_topics_and_themes(messages_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    political_keywords = [
        'fascism', 'fascist', 'theory', 'marxist', 'communist', 'capitalism',
        'dialectical', 'material', 'bourgeois', 'proletariat', 'revolution',
        'nazi', 'zionism', 'imperialism', 'nato', 'cia', 'fbi'
    ]
    
    # Personal/casual keywords  
    personal_keywords = [
        'food', 'cooking', 'eat', 'hungry', 'dinner', 'lunch', 'coffee',
        'sleep', 'tired', 'work', 'job', 'money', 'family', 'friend'
    ]
    
    # Internet culture keywords
    internet_keywords = [
        'lmao', 'omg', 'lol', 'wtf', 'tbh', 'ngl', 'fr', 'bruh',
        'meme', 'tweet', 'twitter', 'tiktok', 'reddit', 'viral'
    ]
    ...
    return {
        ...
        'sample_casual_messages': [msg for msg in messages if any(kw in msg for kw in internet_keywords)][:5]
    }

def analyze_sender_patterns(messages_df: pd.DataFrame, recipients_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for sender_id, stats in sender_stats.items():
        if stats['message_count'] > 0:
            ...
            stats['sender_name'] = sender_lookup.get(sender_id, {}).get('profile_given_name', f'User_{sender_id}')
    
    return dict(sender_stats)

def generate_conversation_report(messages_df: pd.DataFrame, recipients_df: pd.DataFrame, reactions_df: pd.DataFrame) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print(f"✅ Loaded {len(messages_df)} messages, {len(recipients_df)} recipients, {len(reactions_df)} reactions")
