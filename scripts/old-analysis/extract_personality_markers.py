...
def extract_linguistic_patterns(messages: list[str]) -> dict[str, Any]:
    ...
    filler_counts = {
        word: sum(msg.lower().count(word) for msg in messages) for word in filler_words
    }
...
def analyze_conversation_adaptation(messages_df: pd.DataFrame, sender_id: int) -> dict[str, Any]:
    ...
    for thread_id in sender_messages["thread_id"].unique():
        ...
        other_senders = thread_messages[thread_messages["from_recipient_id"] != sender_id][
            "from_recipient_id"
        ].unique()
        ...
        messages_text = [
            msg
            for msg in sender_thread_msgs["body"].fillna("")
        ...
        for i in range(1, len(sorted_thread)):
            if (
                sorted_thread.iloc[i]["from_recipient_id"] == sender_id
                and sorted_thread.iloc[i - 1]["from_recipient_id"] != sender_id
            ):
                response_time = (
                    sorted_thread.iloc[i]["date_sent"] - sorted_thread.iloc[i - 1]["date_sent"]
                ) / 1000
...
def extract_time_based_patterns(messages_df: pd.DataFrame, sender_id: int) -> dict[str, Any]:
    ...
    for month in sender_messages["month"].unique():
        ...
        messages_text = [
            msg
            for msg in month_messages["body"].fillna("")
            if isinstance(msg, str) and len(msg.strip()) > 0
        ]
...
def generate_personality_profile(
    messages_df: pd.DataFrame, recipients_df: pd.DataFrame, sender_id: int
) -> dict[str, Any]:
    ...
    # Get sender info
    sender_info = (
        recipients_df[recipients_df["_id"] == sender_id].iloc[0]
        if len(recipients_df[recipients_df["_id"] == sender_id]) > 0
        else {}
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract personality markers from Signal conversations"
    )
    parser.add_argument(
        "--sender-id",
        type=int,
        help="Specific sender ID to analyze (default: analyze all active senders)",
    )
    parser.add_argument(
        "--min-messages", type=int, default=50, help="Minimum messages required for analysis"
    )
    ...
    try:
        ...
        if profiles:
            ...
            for sender_id, profile in profiles.items():
                ...
                # Top slang used
                top_slang = sorted(ling["slang_usage"].items(), key=lambda x: x[1], reverse=True)[
                    :3
                ]
                ...
                # Dominant emotions
                top_emotions = sorted(
                    emot["emotion_frequencies"].items(), key=lambda x: x[1], reverse=True
                )[:3]
                if top_emotions:
