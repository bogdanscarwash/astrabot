...
def final_pattern_analysis():
    ...
    print(f'  Messages starting lowercase: {lowercase_starts} ({lowercase_starts/len(messages)*100:.1f}%)')
    print(f'  Messages without ending punctuation: {no_punctuation} ({no_punctuation/len(messages)*100:.1f}%)')
    ...
    intellectual_words = ['theory', 'analysis', 'fundamentally', 'dialectic', 'consciousness', 'material', 'fascist', 'revolutionary', 'thesis']
    casual_words = ['lol', 'haha', 'omg', 'wtf', 'damn', 'shit', 'fuck']
    affectionate_words = ['love', 'beautiful', 'sweet', 'gorgeous', 'cute', 'adorable', 'baby', 'honey']
    
    intellectual_count = sum(1 for msg in messages for word in intellectual_words if word in msg.lower())
    casual_count = sum(1 for msg in messages for word in casual_words if word in msg.lower())
    affectionate_count = sum(1 for msg in messages for word in affectionate_words if word in msg.lower())
