...
from src.core.conversation_processor import preserve_conversation_dynamics
...
class TrainingDataCreator:
    ...
    def create_conversation_windows(
        self, messages_df: pd.DataFrame, window_size: int = 5
    ) -> list[dict]:
        """
        ...
        for thread_id in messages_df["thread_id"].unique():
            ...
            for i in range(len(thread_messages) - 1):
                ...
                for j in range(1, len(context_messages)):
                    time_gap = (
                        context_messages.iloc[j]["date_sent"]
                        - context_messages.iloc[j - 1]["date_sent"]
                    ...
                    context.append(
                        {
                            "speaker": (
                                "You"
                                if msg["from_recipient_id"] == self.your_recipient_id
                                else "Other"
                            ),
                            "text": msg["body"],
                ...
                window = {
                    ...
                    "metadata": {
                        ...
                        "response_delay": (
                            your_response["date_sent"] - context_messages.iloc[-1]["date_sent"]
                        )
                        / 1000,
                    },
                }
    ...
    def segment_natural_dialogues(
        self, messages_df: pd.DataFrame, time_gap_minutes: int = 30
    ) -> list[dict]:
        """
        ...
        for thread_id in messages_df["thread_id"].unique():
            ...
            for i in range(1, len(thread_messages)):
                ...
                # Check time gap
                time_gap = (current_msg["date_sent"] - prev_msg["date_sent"]) / (
                    1000 * 60
                )  # to minutes
            ...
            for episode in episodes:
                ...
                {msg["from_recipient_id"] for msg in episode}
                your_messages = [
                    msg for msg in episode if msg["from_recipient_id"] == self.your_recipient_id
                ]
                ...
                for msg in episode[1:]:
                    if msg["from_recipient_id"] != current_speaker:
                        turn_pattern.append(
                            (
                                "You" if current_speaker == self.your_recipient_id else "Other",
                                turn_count,
                            )
                        )
                        ...
                        turn_count += 1
                turn_pattern.append(
                    ("You" if current_speaker == self.your_recipient_id else "Other", turn_count)
                )
                ...
                episode_data = {
                    ...
                    "metadata": {
                        ...
                        "duration_minutes": (episode[-1]["date_sent"] - episode[0]["date_sent"])
                        / (1000 * 60),
                        "your_message_count": len(your_messages),
                        "turn_pattern": turn_pattern,
                        "initiated_by": (
                            "You"
                            if episode[0]["from_recipient_id"] == self.your_recipient_id
                            else "Other"
                        ),
                        "ended_by": (
                            "You"
                            if episode[-1]["from_recipient_id"] == self.your_recipient_id
                            else "Other"
                        ),
                    },
                }
    ...
    def analyze_personal_texting_style(self, messages_df: pd.DataFrame) -> dict:
        ...
        style_analysis = {
            ...
            "preferred_length": (
                "lengthy" if your_messages["body"].str.len().mean() > 100 else "concise"
            ),
    ...
    def _analyze_emoji_usage(self, messages: pd.DataFrame) -> dict:
        ...
        messages_with_emojis = (
            messages["body"].str.contains(emoji_pattern, regex=True, na=False).sum()
        )
        ...
        return {
            ...
            "emoji_usage_rate": (
                f"{(messages_with_emojis / total_messages * 100):.1f}%"
                if total_messages > 0
                else "0%"
            ),
        }
    ...
    def create_training_examples(
        ...
        text_messages = messages_df[
            (messages_df["body"].notna()) & (messages_df["body"].str.len() > 5)
        ].copy()

        # Create recipient lookup for names
        recipient_lookup = (
            recipients_df.set_index("_id")["profile_given_name"].fillna("Unknown").to_dict()
        )
        ...
        for window in conv_windows:
            ...
            context_text = "\n".join(
                [f"{msg['speaker']}: {msg['text']}" for msg in window["context"]]
            )
        ...
        self.segment_natural_dialogues(text_messages)
    ...
    def _enhance_with_twitter_content(
        self, training_examples: list[dict], messages_df: pd.DataFrame
    ) -> list[dict]:
    ...
    def save_training_data(
        self, training_examples: list[dict], output_path: str, style_analysis: Optional[dict] = None
    ):
        """
    ...
    def create_conversational_training_data(
                ...
                messages_data.append(
                    {
                        ...
                        "to_recipient_id": (
                            3 if msg.sender_id == str(your_recipient_id) else your_recipient_id
                        ),
        ...
        raw_examples = create_conversational_training_data(
                    ...
                    chat_examples = [
                        {
                            "messages": [
                                ...
                                {"role": "user", "content": "Share something interesting."},
        ...
        for example in raw_examples:
            ...
            if include_metadata:
                ...
                chat_examples.append({"messages": messages, "metadata": metadata})
            else:
    ...
    def create_burst_sequence_data(
        self, messages: list[any], your_recipient_id: Optional[str] = None
    ) -> list[dict[str, any]]:
        """
        ...
        for i, msg in enumerate(messages):
            ...
            if not current_burst or (
                sender_id == int(current_burst[-1]["sender_id"])
                and (timestamp - current_burst[-1]["timestamp"]).total_seconds() < burst_threshold
            ):
                current_burst.append(
                    {"sender_id": str(sender_id), "timestamp": timestamp, "text": text}
                )
            else:
                # Process previous burst if it's from 'you'
                if (
                    current_burst
                    and int(current_burst[0]["sender_id"]) == your_recipient_id
                    and len(current_burst) > 1
                ):
                    ...
                    training_examples.append(
                        {
                            "messages": [
                                ...
                                {"role": "assistant", "content": burst_text},
                            ],
                            "metadata": {
                                "type": "burst_sequence",
            ...
            training_examples.append(
                {
                    ...
                    "metadata": {"type": "burst_sequence", "burst_size": len(current_burst)},
                }
            )
    ...
    def create_adaptive_training_data(
        ...
        for msg in messages:
            ...
            partner_messages[partner_id].append({"sender_id": sender_id, "text": text})
        ...
        for partner_id, msgs in partner_messages.items():
            if len(msgs) > 2:
                ...
                for i in range(1, len(msgs)):
                    if (
                        msgs[i]["sender_id"] == your_recipient_id
                        and msgs[i - 1]["sender_id"] != your_recipient_id
                    ):
                        ...
                        training_examples.append(
                            {
                                ...
                                "metadata": {"type": "adaptive", "partner_id": partner_id},
                            }
                        )
                ...
                training_examples.append(
                    {
                        ...
                        "metadata": {"type": "qa", "has_question_mark": True},
                    }
                )
...
def create_conversational_training_data(
    messages_df: pd.DataFrame, recipients_df: pd.DataFrame, your_recipient_id: int = 2
) -> list[dict[str, any]]:
    """
    ...
    conv_windows = create_conversation_windows(
        text_messages, window_size=5, your_recipient_id=your_recipient_id
    )

    for window in conv_windows:
        # Format context as natural conversation
        context_text = "\n".join([f"{msg['speaker']}: {msg['text']}" for msg in window["context"]])
    ...
    episodes = segment_natural_dialogues(
        text_messages, time_gap_minutes=30, your_recipient_id=your_recipient_id
    )
    ...
    for pattern in role_patterns:
        ...
        context_text = "\n".join([f"{msg['speaker']}: {msg['text']}" for msg in pattern["context"]])
    ...
    for dynamic in dynamics:
        ...
        if dynamic["style"] == "burst_sequence":
            ...
            context_text = "\n".join(
                [f"{msg['speaker']}: {msg['text']}" for msg in dynamic["context"]]
            )
