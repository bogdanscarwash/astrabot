...
class PrivacyFilter:
    ...
    def anonymize_text(self, text: str, privacy_level: Optional[PrivacyLevel] = None) -> str:
        ...
        for pattern in self.patterns:
            if privacy_level is None or pattern.privacy_level.value in [
                PrivacyLevel.CRITICAL.value, PrivacyLevel.HIGH.value
            ]:
                anonymized = re.sub(
                    pattern.pattern, 
                    pattern.replacement, 
    ...
    def filter_dataframe(self, df: pd.DataFrame, field_mapping: Dict[str, PrivacyLevel]) -> pd.DataFrame:
        ...
        for column, privacy_level in field_mapping.items():
            if column in filtered_df.columns:
                if privacy_level == PrivacyLevel.CRITICAL:
                    ...
                    elif column in ['profile_given_name', 'profile_family_name', 'sender_name', 'system_given_name', 'system_family_name']:
    ...
    def create_privacy_report(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for column in df.columns:
            if df[column].dtype == 'object':  # Text columns
                ...
                max_privacy = max(privacy_levels, key=lambda x: ['low', 'medium', 'high', 'critical'].index(x.value))
