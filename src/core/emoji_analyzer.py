...
class EmojiAnalyzer:
    ...
    def analyze_message_emoji_patterns(
        self, message: str, sender_id: str, timestamp: datetime
    ) -> dict[str, Any]:
        ...
        emotion_counts = Counter(
            e["emotion_category"] for e in emojis if e["emotion_category"] != "other"
        )
        dominant_emotion = emotion_counts.most_common(1)[0][0] if emotion_counts else None

        # Calculate emotional intensity (0-1)
        max_abs_sentiment = (
            max(abs(score) for score in sentiment_scores) if sentiment_scores else 0.0
        )
        ...
        return {
            ...
            "unique_emojis": len({e["emoji"] for e in emojis}),
    ...
    def detect_emoji_signatures(
        self, messages_df: pd.DataFrame, min_usage: int = 5
    ) -> dict[str, list[EmojiUsagePattern]]:
            ...
            for emoji_char, count in emoji_counts.items():
                if count >= min_usage:
                    ...
                    most_common_position = (
                        position_types.most_common(1)[0][0] if position_types else "mid_message"
                    )

                    # Determine if this is a signature emoji (used significantly more than average)
                    is_signature = (
                        count >= min_usage and frequency_rate > 0.05
                    )  # 5% or more of messages
    ...
    def analyze_emotional_state_correlation(
        self, messages_df: pd.DataFrame, time_window_minutes: int = 30
    ) -> dict[str, Any]:
            ...
            for i in range(len(sender_messages)):
                ...
                for _, msg in window_messages.iterrows():
                    emoji_analysis = self.analyze_message_emoji_patterns(
                        msg.get("body", ""), sender_id, msg["datetime"]
                    )
    ...
    def generate_emoji_insights(self, messages_df: pd.DataFrame) -> dict[str, Any]:
        ...
        insights = {
            "usage_statistics": {
                ...
                "emoji_usage_rate": (
                    messages_with_emojis / total_messages if total_messages > 0 else 0
                ),
    ...
    def interpret_emoji_in_context(
        self, emoji_char: str, message_context: str, sender_history: Optional[list[str]] = None
    ) -> dict[str, Any]:
