...
from src.models.schemas import TopicCategory, EmotionalTone
from src.models.conversation_schemas import TopicTransition, ConversationDynamics
...
class TopicTracker:
    ...
    def _init_topic_keywords(self) -> None:
        ...
        self.topic_keywords = {
            TopicCategory.POLITICAL_THEORY: {
                'primary': [
                    'dialectical', 'materialist', 'marxist', 'communist', 'capitalism',
                    'bourgeois', 'proletariat', 'class struggle', 'surplus value',
                    'commodity', 'labor power', 'means of production', 'base and superstructure',
                    'ideology', 'hegemony', 'praxis', 'revolutionary', 'revolution'
                ],
                'secondary': [
                    'theory', 'analysis', 'framework', 'paradigm', 'methodology',
                    'thesis', 'antithesis', 'synthesis', 'critique', 'discourse'
                ]
            },
            ...
            TopicCategory.POLITICS: {
                ...
                'secondary': [
                    'government', 'political', 'vote', 'campaign', 'candidate',
                    'party', 'politician', 'democratic', 'republic'
                ]
            },
            ...
            TopicCategory.CURRENT_EVENTS: {
                'primary': [
                    ...
                    'update', 'development', 'situation', 'event', 'report',
                    'intelligence', 'cia', 'fbi', 'nsa', 'surveillance', 'classified'
                ],
                'secondary': [
                    'media', 'press', 'journalist', 'article', 'story', 'coverage'
                ]
            },
            
            TopicCategory.FOOD: {
                'primary': [
                    'food', 'cooking', 'recipe', 'eat', 'eating', 'dinner', 'lunch', 'breakfast',
                    'restaurant', 'kitchen', 'cook', 'meal', 'hungry', 'delicious',
                    'taste', 'flavor', 'spice', 'arepas', 'coffee', 'drink', 'drinking'
                ],
                'secondary': [
                    'chef', 'cuisine', 'dish', 'ingredient', 'bake', 'baking',
                    'grill', 'grilling', 'fry', 'boil', 'steam'
                ]
            },
            
            TopicCategory.PERSONAL_LIFE: {
                'primary': [
                    'work', 'job', 'career', 'family', 'friend', 'friends', 'relationship',
                    'home', 'house', 'travel', 'vacation', 'health', 'exercise',
                    'sleep', 'tired', 'money', 'weekend', 'today', 'tomorrow'
                ],
                'secondary': [
                    'personal', 'private', 'life', 'living', 'daily', 'routine'
                ]
            },
            ...
            TopicCategory.TECHNOLOGY: {
                'primary': [
                    ...
                    'phone', 'smartphone', 'laptop', 'server', 'database'
                ],
                'secondary': [
                    'technical', 'system', 'platform', 'online', 'cyber', 'virtual'
                ]
            },
            ...
            TopicCategory.ACADEMIC: {
                'primary': [
                    ...
                    'university', 'college', 'professor', 'student', 'academic'
                ],
                'secondary': [
                    'intellectual', 'scholarly', 'theoretical', 'analytical', 'critical'
                ]
            },
            
            TopicCategory.ENTERTAINMENT: {
                'primary': [
                    'movie', 'film', 'tv', 'show', 'television', 'music', 'song', 'band', 'artist',
                    'game', 'gaming', 'book', 'read', 'reading', 'novel', 'story'
                ],
                'secondary': [
                    'entertainment', 'fun', 'enjoy', 'watch', 'listen', 'play'
                ]
            },
            ...
            TopicCategory.MEMES: {
                ...
                'secondary': [
                    'funny', 'hilarious', 'joke', 'humor', 'comedy', 'ridiculous'
                ]
            },
            
            TopicCategory.SOCIAL_MEDIA: {
                'primary': [
                    'twitter', 'tweet', 'retweet', 'tiktok', 'instagram', 'facebook',
                    'reddit', 'youtube', 'social media', 'post', 'share', 'viral'
                ],
                'secondary': [
                    'follow', 'follower', 'like', 'comment', 'subscribe', 'tag'
                ]
            }
        }
    ...
    def detect_message_topics(self, message: str, context_messages: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        academic_count = sum(1 for indicator in self.academic_indicators if indicator in message_lower)
        if academic_count >= 2:
        ...
        if urls:
            for url in urls:
                ...
                elif any(platform in domain for platform in ['youtube.com', 'youtu.be', 'tiktok.com']):
    ...
    def track_topic_transitions(self, messages_df: pd.DataFrame, time_threshold_minutes: int = 15) -> List[TopicTransition]:
        ...
        for thread_id in messages_df['thread_id'].unique():
            ...
            for _, message in thread_messages.iterrows():
                ...
                if (previous_topic and 
                    current_topic != previous_topic and 
                    current_topic != TopicCategory.OTHER and
                    topic_analysis['confidence'] > 0.3):
                    
                    # Calculate time gap
                    time_gap = (timestamp - previous_time).total_seconds() / 60 if previous_time else 0
    ...
    def _classify_transition_method(self, message: str, topic_analysis: Dict, time_gap: float, threshold: int) -> str:
    ...
    def _calculate_transition_smoothness(self, from_topic: TopicCategory, to_topic: TopicCategory, 
                                       method: str, time_gap: float) -> float:
    ...
    def analyze_conversation_topics(self, messages_df: pd.DataFrame, window_size: int = 10) -> Dict[str, Any]:
        ...
        for _, message in messages_df.iterrows():
            ...
            if topic_analysis['confidence'] > 0.3:
                ...
                if (primary_topic == TopicCategory.ACADEMIC or 
                    topic_analysis['academic_indicators'] >= 2):
    ...
    def _calculate_topic_engagement(self, topic_by_sender: Dict) -> Dict[str, float]:
        ...
        for sender_id, topics in topic_by_sender.items():
            ...
            political_messages = (
                topics.get(TopicCategory.POLITICS, 0) + 
                topics.get(TopicCategory.POLITICAL_THEORY, 0)
            )
    ...
    def generate_topic_summary(self, topic_analysis: Dict[str, Any]) -> str:
        ...
        political_count = distribution.get(TopicCategory.POLITICS, 0) + distribution.get(TopicCategory.POLITICAL_THEORY, 0)
        total_messages = sum(distribution.values())
        if political_count > 0:
            political_pct = (political_count / total_messages) * 100
            summary_parts.append(f"Political discussions comprise {political_pct:.1f}% of conversations")
        
        # Academic content
        academic_count = len(topic_analysis.get('academic_discussions', []))
        if academic_count > 0:
            summary_parts.append(f"{academic_count} messages contain academic or theoretical language")
        
        # Transitions
        if transitions.get('total_transitions', 0) > 0:
            avg_smoothness = transitions.get('avg_transition_smoothness', 0)
            smoothness_desc = "smooth" if avg_smoothness > 0.6 else "abrupt" if avg_smoothness < 0.4 else "moderate"
