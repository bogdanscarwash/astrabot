...
class MetadataEnricher:
    ...
    def enrich_message(
        self, message: dict[str, Any], extract_link_previews: bool = False
    ) -> dict[str, Any]:
        """
        ...
        if "sentiment" in enrichment:
            ...
            enrichment["sentiment"] = {"score": score, "label": sentiment_value}
...
def add_reaction_context(
    messages_df: pd.DataFrame, reactions_df: Optional[pd.DataFrame] = None
) -> pd.DataFrame:
    """
    ...
    reaction_summary = (
        reactions_df.groupby("message_id")
        .agg({"emoji": lambda x: list(x), "author_id": "count"})
        .rename(columns={"author_id": "reaction_count"})
    )

    # Add reaction data to messages
    messages_df = messages_df.merge(reaction_summary, left_on="_id", right_index=True, how="left")
...
def add_group_context(
    ...
    threads_df: Optional[pd.DataFrame] = None,
...
def add_temporal_context(messages_df: pd.DataFrame) -> pd.DataFrame:
    ...
    messages_df["response_delay"] = (
        messages_df.groupby("thread_id")["date_sent"].diff() / 1000
    )  # Convert to seconds
...
def add_conversation_flow_metadata(
    ...
    messages_df["consecutive_count"] = (
        messages_df.groupby(["thread_id", (messages_df["speaker_changed"].cumsum())]).cumcount() + 1
    )

    # Identify if this is part of a burst
    messages_df["is_burst_message"] = (messages_df["consecutive_count"] > 1) | (
...
def enrich_messages_with_all_metadata(
    messages_df: pd.DataFrame,
