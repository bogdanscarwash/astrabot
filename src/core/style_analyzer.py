...
class StyleAnalyzer:
    ...
    def analyze_all_communication_styles(
        self, messages_df: pd.DataFrame, recipients_df: pd.DataFrame, min_messages: int = None
        ...
        logger.info(
            f"Analyzing communication styles for {len(frequent_contacts)} frequent contacts"
        )
    ...
    def analyze_emoji_usage(self, messages: pd.DataFrame) -> dict[str, Any]:
        ...
        messages_with_emojis = (
            messages["body"].str.contains(self.emoji_pattern, regex=True, na=False).sum()
        )
        ...
        return {
            ...
            "avg_emojis_per_emoji_message": (
                total_emojis / messages_with_emojis if messages_with_emojis > 0 else 0
            ),
    ...
    def analyze_timing_patterns(self, messages: pd.DataFrame) -> dict[str, Any]:
        ...
        weekday_messages = day_distribution[
            day_distribution.index.isin(range(0, 5))
        ].sum()  # Mon-Fri
    ...
    def _classify_activity_pattern(
        self, night: int, morning: int, afternoon: int, evening: int, total: int
    ) -> str:
    ...
    def analyze_response_patterns(
        self, contact_messages: pd.DataFrame, all_messages: pd.DataFrame
    ) -> dict[str, Any]:
        """
        ...
        response_times = []
        contact_id = (
            contact_messages["from_recipient_id"].iloc[0] if len(contact_messages) > 0 else None
        )
        ...
        for thread_id in contact_messages["thread_id"].unique():
            thread_msgs = all_messages[all_messages["thread_id"] == thread_id].sort_values(
                "date_sent"
            )
            ...
            for i in range(len(thread_msgs) - 1):
                ...
                if (
                    current_msg["from_recipient_id"] != contact_id
                    and next_msg["from_recipient_id"] == contact_id
                ):
    ...
    def create_adaptation_context(
        self, current_msg: pd.Series, your_response: pd.Series, other_person_style: dict[str, Any]
    ) -> list[str]:
        """
        ...
        if (
            other_style in ["formal_lengthy_texter", "expressive_lengthy_texter"]
            and your_length > 100
        ):
        ...
        response_delay = (your_response["date_sent"] - current_msg["date_sent"]) / (
            1000 * 60
        )  # minutes
        other_response_speed = other_person_style.get("response_speed", {}).get(
            "response_speed_classification", "unknown"
        )
        ...
        if other_response_speed == "quick_responder" and response_delay < 30:
            ...
            other_response_speed in ["instant_responder", "quick_responder"] and response_delay < 10
        ):
    ...
    def generate_style_summary(
        self, communication_styles: dict[int, dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        # Average metrics
        avg_message_lengths = [
            style["avg_message_length"] for style in communication_styles.values()
        ]
        burst_frequencies = [
            style["burst_patterns"]["burst_frequency"] for style in communication_styles.values()
        ]
        emoji_frequencies = [
            style["emoji_usage"]["emoji_frequency"] for style in communication_styles.values()
        ]

        # Response speed distribution
        response_speeds = []
        for style in communication_styles.values():
            speed_class = style.get("response_speed", {}).get(
                "response_speed_classification", "unknown"
            )
        ...
        return {
            ...
            "most_common_style": (
                style_distribution.most_common(1)[0] if style_distribution else None
            ),
...
def analyze_all_communication_styles(
    messages_df: pd.DataFrame, recipients_df: pd.DataFrame, min_messages: int = 50
) -> dict[int, dict[str, Any]]:
...
def analyze_response_patterns(
    contact_messages: pd.DataFrame, all_messages: pd.DataFrame
) -> dict[str, Any]:
    """Backwards compatibility wrapper."""
    analyzer = StyleAnalyzer()
...
def create_adaptation_context(
    current_msg: pd.Series, your_response: pd.Series, other_person_style: dict[str, Any]
) -> list[str]:
    """Backwards compatibility wrapper."""
    analyzer = StyleAnalyzer()
