...
from src.models.schemas import (
    PersonalityMarkers, EmojiUsagePattern, EmotionalTone, 
    TopicCategory, MessageType, ConversationMood
)
from src.models.conversation_schemas import (
    RelationshipDynamic, ConversationDynamics, MessageTiming, ConversationStyleProfile
)
...
class PersonalityProfiler:
    ...
    def _init_language_patterns(self) -> None:
        ...
        self.academic_patterns = {
            ...
            'analytical_terms': [
                'analysis', 'synthesis', 'framework', 'paradigm', 'methodology',
                'hypothesis', 'empirical', 'theoretical', 'conceptual', 'dialectical'
            ],
            'formal_phrases': [
                'it should be noted', 'one might argue', 'in this context',
                'from this perspective', 'it follows that', 'as previously mentioned'
            ]
        }
        ...
        self.casual_patterns = {
            'contractions': [
                "don't", "can't", "won't", "it's", "you're", "we're", "they're",
                "i'm", "he's", "she's", "that's", "what's", "where's", "how's"
            ],
            'internet_slang': [
                'lmao', 'lol', 'omg', 'wtf', 'tbh', 'ngl', 'fr', 'bruh',
                'imo', 'imho', 'smh', 'fml', 'yolo', 'af', 'lowkey', 'highkey'
            ],
            'filler_words': [
                'like', 'um', 'uh', 'you know', 'i mean', 'basically',
                'literally', 'actually', 'honestly', 'kinda', 'sorta'
            ]
        }
        
        # Profanity patterns (for communication style analysis)
        self.profanity_patterns = [
            r'\bfuck\w*', r'\bshit\w*', r'\bdamn\w*', r'\bhell\w*',
            r'\bass\w*', r'\bbitch\w*', r'\bcrap\w*', r'\bpiss\w*'
        ]
    ...
    def detect_humor_patterns(self, messages: List[str]) -> Dict[str, Any]:
        ...
        for message in messages:
            ...
            if (len(set(re.findall(r'\b\w+\b', message_lower))) < len(message_lower.split()) / 2 and
        ...
        return {
            ...
            'dominant_humor_style': max(humor_scores.items(), key=lambda x: x[1])[0] if humor_scores else None
        }
    ...
    def analyze_messaging_style(self, messages_df: pd.DataFrame, sender_id: str) -> Dict[str, Any]:
        ...
        for i in range(len(sender_messages)):
            if i == 0:
                ...
                time_diff = (sender_messages.iloc[i]['datetime'] - 
                           sender_messages.iloc[i-1]['datetime']).total_seconds()
        ...
        for i in range(1, len(message_texts)):
            ...
            if (len(current) > 10 and len(previous) > 10 and
                current.startswith(previous[:int(len(previous)*0.7)])):
                corrections += 1
        ...
        return {
            ...
            'burst_messaging': {
                ...
                'avg_burst_size': np.mean([len(burst) for burst in burst_sequences]) if burst_sequences else 0,
    ...
    def analyze_relationship_dynamics(self, messages_df: pd.DataFrame, 
                                    sender_id: str, partner_id: str) -> RelationshipDynamic:
        ...
        balance = min(sender_msgs, total_msgs - sender_msgs) / max(sender_msgs, total_msgs - sender_msgs)
    ...
    def generate_personality_profile(self, messages_df: pd.DataFrame, 
                                   sender_id: str, recipients_df: pd.DataFrame) -> PersonalityMarkers:
        ...
        sender_name = sender_info.iloc[0]['profile_given_name'] if len(sender_info) > 0 else f'User_{sender_id}'
        ...
        engagement_scores = topic_analysis.get('topic_engagement_scores', {}).get(str(sender_id), {})
        ...
        signature_phrases = [word for word, count in word_freq.most_common(20) 
    ...
    def generate_conversation_style_profile(self, messages_df: pd.DataFrame,
                                          participant_ids: List[str],
                                          recipients_df: pd.DataFrame) -> ConversationStyleProfile:
        """Generate conversation style profile for a group of participants."""
        logger.info(f"Generating conversation style profile for {len(participant_ids)} participants")
        
        # Filter messages to this group
        group_messages = messages_df[
            messages_df['from_recipient_id'].isin(participant_ids)
        ].copy()
        ...
        political_ratio = (topic_dist.get(TopicCategory.POLITICS, 0) + 
                          topic_dist.get(TopicCategory.POLITICAL_THEORY, 0)) / max(sum(topic_dist.values()), 1)
        humor_ratio = topic_dist.get(TopicCategory.HUMOR, 0) / max(sum(topic_dist.values()), 1)
        academic_ratio = topic_dist.get(TopicCategory.ACADEMIC, 0) / max(sum(topic_dist.values()), 1)
        ...
        return ConversationStyleProfile(
            ...
            emotional_range=[EmotionalTone.HUMOROUS, EmotionalTone.SERIOUS, EmotionalTone.CONTEMPLATIVE],  # Default range
            ...
            personal_sharing_rate=topic_dist.get(TopicCategory.PERSONAL_LIFE, 0) / max(sum(topic_dist.values()), 1)
        )
    
    def analyze_communication_adaptation(self, messages_df: pd.DataFrame, 
                                       sender_id: str) -> Dict[str, ConversationStyleProfile]:
        ...
        for thread_id in sender_messages['thread_id'].unique():
            ...
            other_participants = list(thread_messages[
                thread_messages['from_recipient_id'] != sender_id
