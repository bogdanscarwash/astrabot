...
class SignalMessage(BaseModel):
    ...
    contains_profanity: bool = Field(
        default=False, description="Whether message contains profanity"
    )
    academic_language: bool = Field(
        default=False, description="Whether message uses academic language"
    )
    internet_slang: bool = Field(default=False, description="Whether message uses internet slang")

    # Conversation context
    response_to_message_id: Optional[str] = Field(
        None, description="ID of message this responds to"
    )
    time_since_previous: Optional[float] = Field(None, description="Seconds since previous message")
    is_correction: bool = Field(
        default=False, description="Whether this corrects a previous message"
    )
    is_continuation: bool = Field(
        default=False, description="Whether this continues previous thought"
    )
...
class ConversationWindow(BaseModel):
    ...
    topic_transitions: list[TopicTransition] = Field(
        default_factory=list, description="Topic changes in window"
    )
    ...
    burst_sequences: list[BurstSequence] = Field(
        default_factory=list, description="Rapid message sequences"
    )
        ...
        for msg in self.messages:
            if msg.sender_id == your_recipient_id:
                ...
                context_messages.append(
                    {"speaker": "Other", "text": msg.body, "timestamp": msg.timestamp.isoformat()}
                )
...
class ConversationThread(BaseModel):
    ...
    participant_personalities: dict[str, PersonalityMarkers] = Field(
        description="Personality analysis per participant"
    )
    communication_balance: dict[str, float] = Field(description="Speaking time distribution (0-1)")

    # Behavioral patterns
    typical_response_times: dict[str, MessageTiming] = Field(
        description="Typical response speed per participant"
    )
    emoji_usage_patterns: dict[str, list[EmojiUsagePattern]] = Field(
        description="Emoji patterns per participant"
    )
    url_sharing_frequency: dict[str, int] = Field(description="URL sharing count per participant")

    def get_training_examples(
        self, your_recipient_id: str, style_focus: Optional[str] = None
    ) -> list[dict[str, Any]]:
        ...
        for window in self.windows:
            ...
            if style_focus:
                if (
                    style_focus == "philosophical"
                    and window.conversation_dynamics != ConversationDynamics.PHILOSOPHICAL
                ):
                    continue
                elif style_focus == "casual" and window.conversation_dynamics not in [
                    ConversationDynamics.CASUAL_BANTER,
                    ConversationDynamics.MEDIA_SHARING,
                ]:
                    continue
                elif (
                    style_focus == "political"
                    and window.conversation_dynamics != ConversationDynamics.POLITICAL_DEBATE
                ):
                    continue
...
class ConversationStyleProfile(BaseModel):
    ...
    def generate_style_instructions(self) -> str:
        ...
        if self.burst_messaging_tendency > 0.6:
            instructions.append(
                "Send multiple short messages in quick succession rather than long single messages"
            )
        ...
        if TopicCategory.POLITICS in self.preferred_topics:
            instructions.append(
                "Engage in political and theoretical discussions with depth and nuance"
            )
