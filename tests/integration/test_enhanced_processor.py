...
def test_enhanced_processor():
    ...
    sample_recipients = pd.DataFrame([
        ...
        {'_id': '3', 'profile_given_name': 'Alice'}
    ])
    ...
    for _, row in sample_messages.iterrows():
        signal_msg = processor.process_signal_message(row, context_messages[-3:] if context_messages else None)
    ...
    for i, burst in enumerate(burst_sequences):
        print(f"Burst {i+1}: {burst.message_count} messages, {burst.duration_seconds:.1f}s duration")
        print(f"  Topic: {burst.topic_category}, Tone: {burst.emotional_tone}")
    
    # Test conversation windows
    print("\n--- Testing Conversation Windows ---")
    windows = processor.create_conversation_windows(context_messages, window_size=4, your_recipient_id="2")
