...
class TestPrivacyPipelineIntegration:
        ...
        for category, test_cases in privacy_test_cases.items():
            for test_case in test_cases:
                ...
                if should_match:
                    assert (
                        result == expected
                    ), f"Failed for {category} - {test_case['pattern_type']}: '{input_text}' -> '{result}' != '{expected}'"
                else:
                    # For cases that shouldn't match, result should equal input
                    assert (
                        result == input_text
                    ), f"False positive for {category} - {test_case['pattern_type']}: '{input_text}' was modified to '{result}'"
    ...
    def test_differential_privacy_integration(self, privacy_filter):
        ...
        for col in numeric_columns:
            ...
            assert not private_stats[col].equals(
                conversation_stats[col]
            ), f"No noise detected in column '{col}'"
    ...
    def test_anonymization_validation_pipeline(self, sample_sensitive_data, privacy_filter):
        ...
        for i, validation in enumerate(validation_results):
            ...
            if validation["patterns_found"]:
                assert validation[
                    "is_anonymized"
                ], f"Message not anonymized despite patterns found: '{original}'"
                assert (
                    len(validation["potential_leaks"]) == 0
                ), f"Potential leaks found in: '{original}'"

                # Check that all found patterns were fully anonymized
                for pattern_info in validation["patterns_found"]:
                    assert pattern_info[
                        "fully_anonymized"
                    ], f"Pattern '{pattern_info['pattern']}' not fully anonymized in: '{original}'"
    ...
    def test_conversation_flow_preservation(self, sample_sensitive_data, privacy_filter):
        ...
        assert all(
            len(body.strip()) > 0 for body in anonymized_bodies
        ), "Some messages became empty after anonymization"
    ...
    def test_hash_consistency_across_sessions(self, privacy_filter):
        ...
        first_session_hashes = {
            id_: privacy_filter.hash_identifier(id_) for id_ in test_identifiers
        }

        # Create new filter instance (simulating new session)
        new_privacy_filter = PrivacyFilter(enable_nlp=False)
        second_session_hashes = {
            id_: new_privacy_filter.hash_identifier(id_) for id_ in test_identifiers
        }

        # Verify consistency
        for identifier in test_identifiers:
            assert (
                first_session_hashes[identifier] == second_session_hashes[identifier]
            ), f"Hash inconsistency for '{identifier}'"
        ...
        edge_case_data = pd.DataFrame(
            {
                ...
                "sender_name": ["John Doe", None, "", "Jane Smith", "Bob Johnson"],
            }
        )

        field_mapping = {"body": PrivacyLevel.CRITICAL, "sender_name": PrivacyLevel.CRITICAL}
...
class TestPrivacyFilterWithRealData:
    ...
    def test_privacy_filter_with_quoted_messages(self):
        ...
        privacy_filter = PrivacyFilter(enable_nlp=False)
        field_mapping = {"body": PrivacyLevel.CRITICAL, "quote_body": PrivacyLevel.CRITICAL}
