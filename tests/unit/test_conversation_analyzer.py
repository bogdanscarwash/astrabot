...
class TestConversationAnalyzer:
    ...
    def sample_conversation_df(self):
        ...
        for i in range(10):
            ...
            messages.append({
                ...
                'date_received': int((base_time + timedelta(minutes=i*2, seconds=1)).timestamp() * 1000)
            })
    ...
    def test_segment_natural_dialogues(self, analyzer, sample_conversation_df):
        ...
        episodes = analyzer.segment_natural_dialogues(
            sample_conversation_df,
            time_gap_minutes=30
        )
    ...
    def test_analyze_personal_texting_style(self, analyzer, sample_conversation_df):
        ...
        style = analyzer.analyze_personal_texting_style(
            sample_conversation_df,
            your_recipient_id=2
        )
    ...
    def test_model_conversation_roles(self, analyzer, sample_conversation_df):
        ...
        if len(role_patterns) > 0:
            ...
            assert first_pattern['role'] in [
                'conversation_driver', 'responsive_participant',
                'active_engager', 'balanced_conversationalist'
            ]
    
    def test_empty_conversation_handling(self, analyzer):
        """Test handling of empty conversation data"""
        # Create empty DataFrame with required columns
        empty_df = pd.DataFrame(columns=['thread_id', 'from_recipient_id', 'to_recipient_id', 
