...
from src.models.schemas import TopicCategory, TopicTransition
...
class TestTopicTracker:
    ...
    def test_topic_detection_single_message(self, tracker):
        ...
        # Food
        food_result = tracker.detect_message_topics(
            "This restaurant has the best tacos in town!"
        )
    ...
    def test_confidence_scoring(self, tracker):
        ...
        low_conf_result = tracker.detect_message_topics(
            "That's interesting"
        )
        assert low_conf_result['confidence'] < 0.5
        
        # Medium confidence - mixed topics
        med_conf_result = tracker.detect_message_topics(
            "Reading about stuff online"
        )
    ...
    def test_batch_topic_detection(self, tracker, sample_messages):
        ...
        correct = sum(1 for detected, expected in zip(detected_topics, expected_topics)
    ...
    def test_topic_transition_smoothness(self, tracker):
        ...
        abrupt_messages = pd.DataFrame({
            '_id': [1, 2],
            'body': [
                "The political situation is dire",
                "Want to get pizza for lunch?"
            ],
            'date_sent': [1000000, 1060000]
        })
